...
export class CommitParser extends BaseParser {
    ...
    async parse(): Promise<CommitConfiguration> {
        ...
        const { template_folder, fabric, consensus, channel, chaincode, path_chaincode, version } = parsedYaml['chains'];
        const conf: CommitConfiguration = new CommitConfiguration(this.fullFilePath, channel, chaincode, path_chaincode, version);
