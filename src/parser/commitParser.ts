...
export class CommitParser extends BaseParser {
    ...
    async parse(): Promise<CommitConfiguration> {
        ...
        const { template_folder, channel, chaincode, root_path_chaincode, lang_chaincode, env_chaincode, path_chaincode, endorsementPolicy, version } = parsedYaml['chaincode'];
        const conf: CommitConfiguration = new CommitConfiguration(this.filePath, template_folder, channel, chaincode, root_path_chaincode, lang_chaincode, env_chaincode, path_chaincode, version, endorsementPolicy);
