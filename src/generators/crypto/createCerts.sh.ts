...
fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=${this.options.org.fullName} -m admin -u http://admin:adminpw@ca.interm.${this.options.org.fullName}:8054 

sleep 10

# Register admin identity and peer
fabric-ca-client register --id.name Admin@${this.options.org.fullName} --id.secret mysecret --id.type client --id.affiliation ${this.options.org.name} -u http://ca.interm.${this.options.org.fullName}:8054 

fabric-ca-client register --id.name ${this.options.org.firstPeerFullName} --id.secret mysecret --id.type peer --id.affiliation ${this.options.org.name} -u http://ca.interm.${this.options.org.fullName}:8054 
