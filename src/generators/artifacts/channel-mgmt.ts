...
orderers:
   ...
   * @param peers
   */
  async joinChannel(channelName: string, peers: string[], allPeers: boolean): Promise<boolean> {
    try {
      ...
      // create the provided channel
      const isJoined = await channelClient.joinChannel(channelName, this.network.organizations[0].mspName, peers, allPeers);
      if(!isJoined) {
        e(`Error channel (${channelName}) creation !!!`);
        return false;
      }
