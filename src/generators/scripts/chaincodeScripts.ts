...
approve() {
  if [[ -z "\${ENDORSEMENT}" && -z "\${PRIVATE_COLLECTION}"  ]]; then
    peer lifecycle chaincode approveformyorg -o $CORE_ORDERER_ID --channelID $CHANNEL_NAME --name $CC_NAME --version $VERSION --package-id $PACKAGE_ID --sequence $SEQUENCE --tls --cafile $CORE_ORDERER_TLS_ROOTCERT >&log.txt
        ...
        if [[ -n "\${ENDORSEMENT}" && -z "\${PRIVATE_COLLECTION}" ]]; then
            ...
            peer lifecycle chaincode approveformyorg -o $CORE_ORDERER_ID --channelID $CHANNEL_NAME --name $CC_NAME --version $VERSION --package-id $PACKAGE_ID --sequence $SEQUENCE --tls --cafile $CORE_ORDERER_TLS_ROOTCERT --collections-config "\${PRIVATE_COLLECTION}"  >&log.txt     
            ...
            peer lifecycle chaincode approveformyorg -o $CORE_ORDERER_ID --channelID $CHANNEL_NAME --name $CC_NAME --version $VERSION --package-id $PACKAGE_ID --sequence $SEQUENCE --tls --cafile $CORE_ORDERER_TLS_ROOTCERT --collections-config "\${PRIVATE_COLLECTION}" --signature-policy "\${ENDORSEMENT}" >&log.txt     
        fi
  fi
  cat log.txt
  res=$?
    ...
    while [ $rc -ne 0 -a $COUNTER -lt 1 ] ; do
      ...
      if [[ -z "\${ENDORSEMENT}" && -z "\${PRIVATE_COLLECTION}" ]]; then
        ...
        if [[ -n "\${ENDORSEMENT}" && -z "\${PRIVATE_COLLECTION}" ]]; then
            ...
            echo "______________________Updating private data policy___________________"
            peer lifecycle chaincode checkcommitreadiness --channelID mychannel --name $CC_NAME --version $VERSION --sequence $SEQUENCE --tls --cafile \${CORE_ORDERER_TLS_ROOTCERT} --collections-config "\${PRIVATE_COLLECTION}"  --output json >&log.txt
        elif [[ -n "\${PRIVATE_COLLECTION}" && -n "\${ENDORSEMENT}" ]]; then 
            echo "______________________Updating endorsement and private data policy___________________"
            peer lifecycle chaincode checkcommitreadiness --channelID mychannel --name $CC_NAME --version $VERSION --sequence $SEQUENCE --tls --cafile \${CORE_ORDERER_TLS_ROOTCERT} --collections-config "\${PRIVATE_COLLECTION}" --signature-policy "\${ENDORSEMENT}" --output json >&log.txt
        fi
  ...
  if [[ -z "\${ENDORSEMENT}" && -z "\${PRIVATE_COLLECTION}" ]]; then
      peer lifecycle chaincode commit -o $CORE_ORDERER_ID --tls --cafile \${CORE_ORDERER_TLS_ROOTCERT} --channelID mychannel --name $CC_NAME --version $VERSION --sequence $SEQUENCE \${PEER_TARGETS} >&log.txt
        ...
        if [[ -n "\${ENDORSEMENT}" && -z "\${PRIVATE_COLLECTION}" ]]; then
            ...
            echo "______________________Updating private data policy___________________"
            peer lifecycle chaincode commit -o $CORE_ORDERER_ID --tls --cafile \${CORE_ORDERER_TLS_ROOTCERT} --channelID mychannel --name $CC_NAME --version $VERSION --sequence $SEQUENCE --collections-config "\${PRIVATE_COLLECTION}"  \${PEER_TARGETS} >&log.txt
        elif [[ -n "\${PRIVATE_COLLECTION}" && -n "\${ENDORSEMENT}" ]]; then 
            echo "______________________Updating endorsement and private data policy___________________"
            peer lifecycle chaincode commit -o $CORE_ORDERER_ID --tls --cafile \${CORE_ORDERER_TLS_ROOTCERT} --channelID mychannel --name $CC_NAME --version $VERSION --sequence $SEQUENCE --collections-config "\${PRIVATE_COLLECTION}" --signature-policy "\${ENDORSEMENT}" \${PEER_TARGETS} >&log.txt
        fi
